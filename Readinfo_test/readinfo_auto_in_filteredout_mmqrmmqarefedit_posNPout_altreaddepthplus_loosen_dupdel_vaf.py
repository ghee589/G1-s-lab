...
def result(x, y, ls):
	...
	if len(altreads) == 0:
		...
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "Zero" + " " + str(len(altreads)), "%fsec." % (terminate_time - start_time)
	...
	if checker == 0:
		terminate_time = timeit.default_timer() 
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "P" + " "+str(round(len(altreads)/len(basereads),3)),  "%fsec." % (terminate_time - start_time)

	else:
		terminate_time = timeit.default_timer() 
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3)),  "%fsec." % (terminate_time - start_time)
