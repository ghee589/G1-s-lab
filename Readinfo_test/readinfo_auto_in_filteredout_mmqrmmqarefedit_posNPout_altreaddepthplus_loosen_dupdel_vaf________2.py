...
for l in range(len(chrompos)):
	refalt = fasta.fetch(chrompos[l][0], chrompos[l][1]-1, chrompos[l][1])

	basereads = sequence_read(chrompos[l][0], chrompos[l][1]) #basic read #list form
	...
	if len(altreads) == 0:
		...
		print("Z","%fsec." % (terminate_time - start_time))
		continue
	if len(altreads) == 1:
		terminate_time = timeit.default_timer()
		print("N","%fsec." % (terminate_time - start_time))
		continue
	...
	if mean_mappingQ_refreads - mean_mappingQ_altreads > param2 or mean_mappingQ_altreads < param3 or mean_mappingQ_refreads < param3 or mean_mismatch_altreads > param5 or percent_soft_clipped_sum > param6:
		terminate_time = timeit.default_timer()
		print("N","%fsec." % (terminate_time - start_time))
	else:
		terminate_time = timeit.default_timer()
		print(str([len(altreads), len(basereads)]) +  "_" + "P" + " "+str(round(len(altreads)/len(basereads),3)),"%fsec." % (terminate_time - start_time))
