...
def result(x, y, ls):
	...
	soft_clipped_sum_ref = 0
	for e in range(len(refreads)):
		if refreads[e][3][4] > 0:
			soft_clipped_sum_ref += 1
	...
	percent_soft_clipped_sum_ref = soft_clipped_sum_ref/len(refreads)
	...
	if percent_soft_clipped_sum > param6:
		checker += 1
	if percent_soft_clipped_sum_ref > param7:
		checker += 1
