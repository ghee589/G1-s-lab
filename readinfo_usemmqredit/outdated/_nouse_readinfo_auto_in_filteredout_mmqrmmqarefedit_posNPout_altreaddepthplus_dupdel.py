...
bam = pysam.AlignmentFile(in_file_bam, duplicate_filehandle = False)
...
def result(x, y, ls):
	...
	if len(altreads) == 0:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "Zero"
	...
	if checker == 0:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "P"

	else:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N"
