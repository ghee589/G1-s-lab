...
def result(x, y, ls):
	...
	elif mean_mappingQ_refreads - mean_mappingQ_altreads > param2:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3))

	elif mean_mappingQ_altreads < param3:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3))

	elif mean_mappingQ_refreads < param4:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3))

	elif mean_mismatch_altreads > param5:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3))

	elif percent_soft_clipped_sum > param6:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(round(len(altreads)/len(basereads),3))
