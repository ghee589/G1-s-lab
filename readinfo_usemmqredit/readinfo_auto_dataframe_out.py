...
def result(x, y, ls):
	...
	mean_mappingQ_altreads = round(alt_mappingQ_sum/len(altreads), 2) 

	if len(refreads) == 0:
		mean_mappingQ_refreads = 60
	else:
		mean_mappingQ_refreads = round(ref_mappingQ_sum/len(refreads), 2)

	mean_mismatch_altreads = round(alt_mismatch_sum/len(altreads), 2)
	percent_soft_clipped_sum = round(soft_clipped_sum/len(altreads), 2)
...
####################################################################################################
	...
	return str(supporting_reads_num) + '\t' + str(mean_mappingQ_altreads) + '\t' + str(mean_mappingQ_refreads) + '\t' +  str(abs(mean_mappingQ_altreads - mean_mappingQ_refreads)) + '\t' +  str(mean_mismatch_altreads) + '\t' +  str(percent_soft_clipped_sum)
...
#print('sup_read_sum' + '\t' + 'mean_map_Q_alt' + '\t' + 'mean_map_Q_ref' + '\t' + 'absmean' + '\t' + 'mean_mis_alt' + '\t' + 'softcsum')

for l in range(len(chrompos)):
	print(result(str(chrompos[l][0]), int(chrompos[l][1]), chrompos), sep = '\t') #chrompos is fixed not chrompos[l]
