...
def result(x, y, ls):
	...
	if checker == 0:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "P" + " "+str(len(altreads)/len(basereads))

	else:
		return str((x, y)) + "\t" + str([len(altreads), len(basereads)]) +  "_" + "N" + " "+str(len(altreads)/len(basereads))
